...
type Phase = "resolving" | "resolved" | "browsing" | "planning" | "confirming" | "installing" | "done" | "error";

export function DeepLinkInstallPanel({ request, onDismiss, onInstalled }: Props) {
  ...
  return (
    <div className="skill-install-plan deep-link-panel">
      ...
      {phase === "resolving" && <p className="status-text">正在解析插件信息…</p>}

      {phase === "resolved" && resolution && (
        <>
          <dl>
            {resolution.author && (
              <div><dt>作者</dt><dd>{resolution.author}</dd></div>
            )}
            {resolution.category && (
              <div><dt>分类</dt><dd>{resolution.category}</dd></div>
            )}
            <div><dt>来源</dt><dd>{request.marketplace}</dd></div>
          </dl>
          <div className="skill-install-plan-actions">
            <button className="button button-ghost" onClick={onDismiss}>取消</button>
      ...
      {(phase === "browsing" || phase === "planning" || phase === "installing") && (
      ...
      {phase === "confirming" && plan && (
        <>
          <dl>
            <div><dt>文件</dt><dd>{plan.plan.files.length} 个受管文件</dd></div>
            <div><dt>目标路径</dt><dd className="mono">{plan.plan.targetDirectory}</dd></div>
          </dl>
          <div className="skill-install-plan-actions">
            <button className="button button-ghost" onClick={onDismiss}>取消</button>
            <button className="button button-primary" onClick={handleConfirm}>确认安装</button>
          </div>
        </>
      )}
      ...
      {phase === "done" && (
        <>
          ...
          <div className="skill-install-plan-actions">
            <button className="button button-primary" onClick={onDismiss}>完成</button>
          </div>
        </>
      )}
      ...
      {phase === "error" && (
        <>
          ...
          <div className="skill-install-plan-actions">
            <button className="button button-ghost" onClick={onDismiss}>关闭</button>
          </div>
        </>
      )}
    </div>
