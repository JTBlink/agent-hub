...
备份存放在 Tauri 应用数据目录的 `backups/<config-id>/<timestamp>-<checksum>`，而非原配置旁边或工作空间仓库中。目录在 Unix 使用 `0700`、文件使用 `0600`；Windows 使用当前用户 DACL。SQLite 只保存备份路径和元数据，不保存正文。
