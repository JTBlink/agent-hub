...
- 保留 AgentHub 的三个 Agent 节点、连接线、中心 Hub 标记和蓝绿渐变方向。
- 主体主色可见边界约为 `x = 195–829 px`、`y = 180–837 px`。
- 主体到底板外接矩形的水平安全间距约为 `98 px`，垂直安全间距约为顶部 `83 px`、底部 `90 px`；任一方向不得小于 `80 px`。
- 下方节点到圆角弧线的真实最短距离不得小于 `80 px`，不能只用外接矩形的水平、垂直留白代替圆角距离。
- 中心 Hub、节点和连接线必须整体等比调整，不能单独放大中心标记。
