...
- 使用 Conventional Commits 格式：`<类型>(<范围>): <中文说明>`；范围可省略。
- 类型使用固定英文枚举：`feat`（功能）、`fix`（修复）、`docs`（文档）、`test`（测试）、`build`（构建）、`ci`（持续集成）、`refactor`（重构）、`chore`（杂项）。
- 类型和范围使用小写英文或项目约定短词，说明部分使用中文祈使语气，简洁描述一个明确事项。
- 标题不超过 72 个字符，不以句号结尾。
...
```text
fix(agents): 修复未安装 Agent 的展示状态
test(release): 完善跨平台安装包验收
docs(security): 更新配置安全策略文档
```

涉及不兼容变更时，在类型后添加 `!`，并在正文说明迁移方式，例如：`feat(api)!: 删除旧版接口`。
